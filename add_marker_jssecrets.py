...
backup = f"{path}.bak_addmarkerjs_{ts}"
...
anchor = '''          subject: "[Weekend JS Secrets ${{ matrix.batch }}] ${{ steps.check.outputs.count }} Finding(s)"
...
new = anchor + '''
      ...
      - name: Upload batch completion marker
        ...
        with:
          name: batch-marker-jssecrets-${{ matrix.batch }}
